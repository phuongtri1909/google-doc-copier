            ...
            <div class="card-body">
                ...
                <form action="{{ route('jobs.store') }}" method="POST" class="copy-form-steps">
                    ...
                    <div class="form-step mb-4 animate-on-scroll" data-animation="fadeIn">
                        ...
                        <div class="mb-3 mt-3">
                            <textarea class="form-control @error('source_doc_ids') is-invalid @enderror" 
                                id="source_doc_ids" name="source_doc_ids" rows="4"
                                placeholder="Nhập mỗi ID trên một dòng. Ví dụ:&#10;1aBcDeFgHiJkLmNoPqRsTuVwXyZ&#10;2bCdEfGhIjKlMnOpQrStUvWxYz">{{ old('source_doc_ids') }}</textarea>
                            @error('source_doc_ids')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                            <div class="form-text">
                                <i class="fas fa-info-circle me-1"></i> 
                                ID là phần trong URL tài liệu sau /d/ và trước /edit
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="form-step mb-4 animate-on-scroll delay-2" data-animation="fadeIn">
                        ...
                        <div class="folder-input mt-3 ps-4 folder-existing {{ old('folder_option') == 'new' ? 'd-none' : '' }}">
                            <button type="button" class="btn btn-outline-primary mb-2" id="browse_folders">
                                <i class="fas fa-search me-1"></i> Duyệt Thư Mục Drive
                            </button>
                            <input type="hidden" id="selected_folder_id" name="folder_id" value="{{ old('folder_id') }}">
                            <div id="folder_info" class="p-3 border rounded {{ !old('folder_id') ? 'd-none' : '' }}">
                                ...
                                <input type="hidden" name="folder_name" id="folder_name_hidden" value="{{ old('folder_name') }}">
                            </div>
                        </div>
                        
                        <div class="folder-input mt-3 ps-4 folder-new {{ old('folder_option') == 'new' ? '' : 'd-none' }}">
                    ...
                    <div class="form-step animate-on-scroll delay-3" data-animation="fadeIn">
                        ...
                        <div class="mb-4 mt-3">
                            <label for="interval_seconds" class="form-label">Khoảng thời gian giữa các lần sao chép (giây)</label>
                            <div class="d-flex align-items-center">
                                <input type="range" class="form-range me-3" min="10" max="300" step="5" 
                                       value="{{ old('interval_seconds', 60) }}" id="interval_range">
                                <div class="input-group" style="width: 150px">
                                    <input type="number" class="form-control @error('interval_seconds') is-invalid @enderror" 
                                           id="interval_seconds" name="interval_seconds" value="{{ old('interval_seconds', 60) }}" 
                                           min="10" max="300">
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        document.getElementById('browse_folders').addEventListener('click', function() {
            window.open('{{ route("folders.picker") }}', 'Google Folder Picker', 'width=800,height=600');
        });
